...
import { getPolicyPlan, planColors } from "../constants/plan";
...
export default function GetQuotePage({user}) {
    ...
    try {
        ...
        if (res.ok) {
            ...
            navigate("/dashboard"); // ✅ navigate after successful API call
        } else {
            setResult(computeMockResult(payload));
            navigate("/dashboard"); // ✅ navigate even on fallback
        }
    } catch {
        setResult(computeMockResult(payload));
        navigate("/dashboard"); // ✅ navigate even on error fallback
    }
    setLoading(false);
};
